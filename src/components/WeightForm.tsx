...
const WeightForm: React.FC<WeightFormProps> = ({ onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (cw <= 0 || tw <= 0) {
      alert("Weight values must be greater than 0.");
      return;
    }

    if (cw < 20 || cw > 500) {
      alert("Current weight must be between 20 and 500 kg.");
      return;
    }

    if (tw < 20 || tw > 500) {
      alert("Target weight must be between 20 and 500 kg.");
      return;
    }

    if (Math.abs(cw - tw) > 200) {
      alert("The difference between current and target weight seems unrealistic. Please check your values.");
      return;
    }
