...
const TOTAL_CUPS = 8;
const STORAGE_KEY_WATER = "farfit-water-cups";
...
const WaterTracker: React.FC = () => {
  ...
  // Load water data from localStorage on mount
  useEffect(() => {
    try {
      const savedCups = localStorage.getItem(STORAGE_KEY_WATER);
      if (savedCups) {
        const parsedCups = parseInt(savedCups, 10);
        if (!isNaN(parsedCups) && parsedCups >= 0 && parsedCups <= TOTAL_CUPS) {
          setCups(parsedCups);
        }
      }
    } catch (error) {
      console.error('Error loading water data from localStorage:', error);
    }
  }, []);

  // Save water data to localStorage whenever cups change
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY_WATER, cups.toString());
    } catch (error) {
